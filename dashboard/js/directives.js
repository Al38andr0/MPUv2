...
angular.module("mpuDashboard").directive('categorie', function(){
    return {
        restrict : 'E',
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            console.log($scope.categoriaOriginal)
            $scope.$watch('categoriaOriginal', function(newVal){
                $scope.cat = angular.copy(newVal.categoria);
                $scope.cat.r = '../dashboard/archivio_dati/Categorie/' + $scope.cat.i + '.jpg';
            });
            ...
            $scope.updateCheck = function() {
                return(!$scope.cat.n || !$scope.cat.r || !$scope.cat.x || isNaN($scope.cat.y));
            };

            $scope.newItem = function(){
                $scope.categoriaOriginal.selected = false;
            ...
            $scope.action = function(type) {
                if(type == 'U'){
                    ...
                    $http.post('php/categorie.php?type=update', {
                            'id' : $scope.cat.i,
                            'nome': $scope.cat.n,
                            'pos': $scope.cat.y,
                            'show': $scope.cat.v,
                            ...
                            _.each($scope.vm.categorie, function(v) {
                                if(v.i == $scope.cat.i){
                                    v.n = $scope.cat.n;
                                    v.y = $scope.cat.y;
                                    v.v = $scope.cat.v;
                ...
                if(type == 'D'){
                    $rootScope.saving = true;
                    $http.post('php/categorie.php?type=delete', {
                            'id' : $scope.cat.i,
                            'nome': $scope.cat.n,
                            'pos': $scope.cat.y,
                            'show': $scope.cat.v,
                            ...
                            //console.log('Risposta dalla pagina PHP', data);
                            $scope.vm.categorie = _.filter($scope.vm.categorie, function(list){
...
angular.module("mpuDashboard").directive('settori', function(){
    return {
        restrict : 'E',
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            $scope.$watch('settoreOriginal', function(newVal){
                $scope.set = angular.copy(newVal.settore);
                $scope.set.r = '../dashboard/archivio_dati/Settori/' + $scope.set.i + '.jpg';
            });
            ...
            $scope.updateCheck = function() {
                return(!$scope.set.n || !$scope.set.x || isNaN($scope.set.y));
            };

            $scope.newSettore = function(){
                $scope.settoreOriginal.selected = false;
                $rootScope.saving = false;
            ...
            $scope.action = function(type) {
                if(type == 'U'){
                    $rootScope.saving = true;
                    $http.post('php/settori.php?type=update', {
                            'id' : $scope.set.i,
                            'nome': $scope.set.n,
                            'pos': $scope.set.y,
                            'show': $scope.set.v,
                ...
                if(type == 'D'){
                    $rootScope.saving = true;
                    $http.post('php/settori.php?type=delete', {
                            'id' : $scope.set.i,
                            'nome': $scope.set.n,
                            'pos': $scope.set.y,
                            'show': $scope.set.v,
                            ...
                            //console.log('Risposta dalla pagina PHP', data);
                            $scope.vm.settori = _.filter($scope.vm.settori, function(list){
                                return list.i != $scope.set.i;
                            });
                            $scope.vm.saveSettoriData();
                            $scope.newSettore();
                        }).error(function(data, status) {
...
angular.module("mpuDashboard").directive('marchi', function(){
    return {
        restrict : 'E',
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            $scope.$watch('marchioOriginal', function(newVal){
                $scope.mark = angular.copy(newVal.marchio);
            });
            ...
            $scope.selectedCat = function(ID) {
                return ($scope.mark.g.indexOf(ID) != -1);
            };

            $scope.toggleSet = function(ID) {
                if($scope.mark.g.indexOf(ID) == -1) {
                    $scope.mark.g.push(parseInt(ID));
                } else {
                    $scope.mark.g.splice($scope.mark.g.indexOf(ID), 1);
                }
            };

            $scope.updateCheck = function() {
                return(!$scope.mark.n || $scope.mark.g.length == 0 || isNaN($scope.mark.b) || isNaN($scope.mark.s));
            };

            $scope.deleteCheck = function() {
                return($scope.mark.n !== $scope.marchioOriginal.marchio.n);
            };

            var checkMarchio = function() {
                if($scope.mark.n !== $scope.marchioOriginal.marchio.n){
                    return _.findWhere($scope.vm.marchi, {n : $scope.mark.n});
                } else {
                    return false
                }
            };
            ...
            $scope.newMarchio = function() {
                $scope.marchioOriginal.selected = false;
                $rootScope.saving = false;
            ...
            $scope.action = function(type) {
                if(type == 'U'){
                    if(!checkMarchio()){
                        $rootScope.saving = true;
                        $http.post('php/marchi.php?type=update', {
                                'id' : $scope.mark.i,
                                'nome': $scope.mark.n,
                                'show': $scope.mark.v,
                                //'cat' : $scope.mark.g,
                                ...
                                _.each($scope.vm.marchi, function(v) {
                                    if(v.i == $scope.mark.i){
                                        ...
                                        v.b = $scope.mark.b;
                                        v.s = $scope.mark.s;
                                        //v.g_array = $scope.mark.g;
                                        $scope.mark = angular.copy(v);
                                    }
                                    return $scope.vm.marchi;
                                });
                                $scope.errore.marchio = false;
                                $rootScope.saving = false;
                            }).error(function(data, status) {
                ...
                if(type == 'D'){
                    $rootScope.saving = true;
                    $http.post('php/marchi.php?type=delete', {
                            'id' : $scope.mark.i,
                            'nome': $scope.mark.n,
                            'show': $scope.mark.v,
                            //'cat' : $scope.mark.g,
                            ...
                            //console.log('Risposta dalla pagina PHP', data);
                            $scope.vm.marchi = _.filter($scope.vm.marchi, function(list){
                                return list.i != $scope.mark.i;
                            });
                            $scope.vm.saveMarchiData();
                            $scope.newMarchio();
                        }).error(function(data, status) {
...
angular.module("mpuDashboard").directive('tbpr', function(){
    return {
        restrict : 'E',
        templateUrl : 'template/marchiTabelle.html',
        scope : {
            tbprOriginal : "=",
            vm : "="
        },
        controller : function($scope, $http, $timeout, $rootScope){
            $scope.$watch('tbprOriginal', function(newVal){
                $scope.tbpr = angular.copy(newVal.tbpr);
            });
            ...
            $scope.updateCheck = function() {
                return(!$scope.tbpr.n || isNaN($scope.tbpr.y));
            };

            $scope.copyCheck = function() {
                return(!$scope.tbpr.n || isNaN($scope.tbpr.y) || ($scope.tbpr.n === $scope.tbprOriginal.tbpr.n && $scope.tbpr.m === $scope.tbprOriginal.tbpr.m));
            };

            $scope.newTbpr = function() {
                $scope.tbprOriginal.selected = false;
                $rootScope.saving = false;
            ...
            var checkTBpr = function() {
                if($scope.tbpr.n != $scope.tbprOriginal.tbpr.n || $scope.tbpr.m != $scope.tbprOriginal.tbpr.m) {
                    return _.findWhere($scope.vm.tbpr, {n : $scope.tbpr.n, m : $scope.tbpr.m});
                } else {
                    return false;
                }
            };
            ...
            $scope.action = function(type) {
                if(type == 'U'){
                    if(!checkTBpr()){
                        $rootScope.saving = true;
                        $http.post('php/marchiTabelle.php?type=update', {
                                'id' : $scope.tbpr.i,
                                'nome': $scope.tbpr.n,
                                'pos': $scope.tbpr.y,
                                'mark' : $scope.tbpr.m
                                ...
                                //console.log('Risposta dalla pagina PHP', data);
                                _.each($scope.vm.tbpr, function(v) {
                                    if(v.i == $scope.tbpr.i){
                                        v.n = $scope.tbpr.n;
                                        v.m = $scope.tbpr.m;
                                        v.y = $scope.tbpr.y;
                                        $scope.tbpr = angular.copy(v);
                                    }
                                });
                                $rootScope.saving = false;
                                $scope.errore.tbpr = false;
                ...
                if(type == 'C'){
                    if(!checkTBpr()){
                        ...
                        $http.post('php/marchiTabelle.php?type=new', {
                                ...
                                //console.log('Risposta dalla pagina PHP', data);
                                $scope.vm.copiedTbpr = {
                                    i      :   $scope.vm.nuovaTbpr.i,
                                    n    :   $scope.tbpr.n,
                                    y     :   $scope.tbpr.y,
                                    m    :   $scope.tbpr.m
                                };
                                $scope.vm.tbpr.push($scope.vm.copiedTbpr);
                                $scope.vm.saveTbprData();
                                $scope.vm.nuovaTbpr = {
                                    i : $scope.vm.nuovaTbpr.i + 1
                                };
                ...
                if(type == 'D'){
                    $rootScope.saving = true;
                    $http.post('php/marchiTabelle.php?type=delete', {
                            'id' : $scope.tbpr.i,
                            'nome': $scope.tbpr.n,
                            'pos': $scope.tbpr.y,
                            'mark' : $scope.tbpr.m
                            ...
                            //console.log('Risposta dalla pagina PHP', data);
                            $scope.vm.tbpr = _.filter($scope.vm.tbpr, function(list){
                                return list.i != $scope.tbpr.i;
                            });
                            $scope.vm.saveTbprData();
                            $scope.newTbpr();
                        }).error(function(data, status) {
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            ...
            var checkFinitura = function() {
                if($scope.finitura.n != $scope.finituraOriginal.finitura.n || $scope.finitura.c != $scope.finituraOriginal.finitura.c || $scope.finitura.m != $scope.finituraOriginal.finitura.m) {
                    return _.findWhere($scope.vm.finiture, {n : $scope.finitura.n, c : $scope.finitura.c, m : $scope.finitura.m});
                } else {
                    return false;
                }
            };
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            ...
            var checkUpdate = function() {
                if($scope.finituraTabella.n != $scope.finituraTabellaOriginal.finituraTabella.n || $scope.finituraTabella.m != $scope.finituraTabellaOriginal.finituraTabella.m || $scope.finituraTabella.l != $scope.finituraTabellaOriginal.finituraTabella.l) {
                    return _.findWhere($scope.vm.finitureTabelle, {n : $scope.finituraTabella.n, m : $scope.finituraTabella.m, l : $scope.finituraTabella.l});
                } else {
                    return false;
                }
            };
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            ...
            function checkUpdate() {
                if($scope.abbinamento.c != $scope.abbinamentoOriginal.abbinamento.c || $scope.abbinamento.m != $scope.abbinamentoOriginal.abbinamento.m || $scope.abbinamento.l != $scope.abbinamentoOriginal.abbinamento.l) {
                    return _.findWhere($scope.vm.abbinamenti, {n : $scope.abbinamento.c, m : $scope.abbinamento.m, l : $scope.abbinamento.l});
                } else {
                    return false;
                }
            }
            ...
            function checkCopy() {
                return _.findWhere($scope.vm.abbinamenti, {c : $scope.abbinamento.c, m : $scope.abbinamento.m, l : $scope.abbinamento.l, u : $scope.abbinamento.u});
            }
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            ...
            var checkLineaSettore = function() {
                if($scope.lineaSettore.m != $scope.lineaSettoreOriginal.lineaSettore.m || $scope.lineaSettore.l != $scope.lineaSettoreOriginal.lineaSettore.l || $scope.lineaSettore.h != $scope.lineaSettoreOriginal.lineaSettore.h) {
                    return _.findWhere($scope.vm.lineeSettori, {m : $scope.lineaSettore.m, l : $scope.lineaSettore.l, h : $scope.lineaSettore.h});
                } else {
                    return false;
                }
            };
...
angular.module("mpuDashboard").directive('prodotti', function(){
    return {
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            ...
            var checkCopy = function() {
                return _.findWhere($scope.vm.prodotti, {c : $scope.prodotto.c, m : $scope.prodotto.m, l : $scope.prodotto.l});
            };
        ...
        controller : function($scope, $http, $timeout, $rootScope, dataSvc){
            ...
            $scope.$watch('prodottoSettore', function(newVal){
                ...
                _.each(prodottiPerLinea, function(v){
                    if(!_.find($scope.vm.prodottiSettori, function(list) { return list.c === v.i}))
...
angular.module("mpuDashboard").directive('province', function(){
    return {
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            ...
            var checkExist = function() {
                return _.findWhere($scope.vm.province, {n : $scope.provincia.n, c : $scope.provincia.c, z : $scope.provincia.z, r : $scope.provincia.r});
            };
...
angular.module("mpuDashboard").directive('esposizioni', function(){
    return {
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            ...
            var checkCopy = function() {
                return _.findWhere($scope.vm.esposizioni, {c : $scope.esposizione.c, m : $scope.esposizione.m, l : $scope.esposizione.l, f : $scope.esposizione.f});
            };
        ...
        controller : function($scope, $http, $timeout, $rootScope){
            ...
            var checkUpdate = function() {
                if($scope.composto.c != $scope.compostoOriginal.composto.c || $scope.composto.l != $scope.compostoOriginal.composto.l || $scope.composto.f != $scope.compostoOriginal.composto.f) {
                    return _.findWhere($scope.vm.composti, {c : $scope.composto.c, l : $scope.composto.l, f : $scope.composto.f});
                } else {
                    return false;
                }
            };
            ...
            var checkCopy = function() {
                return _.findWhere($scope.vm.composti, {c : $scope.composto.c, m : $scope.composto.m, l : $scope.composto.l, f : $scope.composto.f});
            };
        ...
        controller : function($scope, $http, $timeout, $rootScope, dataSvc){
            ...
            $scope.$watch('esposizioneSettore', function(newVal){
                ...
                _.each(esposizioniPerLinea, function(v){
                    if(!_.find($scope.vm.esposizioniSettori, function(list) { return (list.c === v.i && newVal.l === v.l) }))
    ...
    return {
        ...
        controller : function($scope, $http, $timeout, $rootScope, dataSvc){
            ...
            $scope.$watch('compostoSettore', function(newVal){
                ...
                _.each(esposizioniPerLinea, function(v){
                    if(!_.find($scope.vm.compostiSettori, function(list) { return (list.c === v.i && newVal.l === v.l) }))
